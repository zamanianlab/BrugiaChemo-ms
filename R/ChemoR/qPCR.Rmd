...
results <- read_delim(str_glue(here("data", "qpcr_results.txt")),
                      col_names = TRUE, 
                      delim = "\t", 
                      skip = 8) %>%
  clean_names() %>%
...
table <- pivot_wider(ungroup(ct_df), names_from = "target_name", values_from = "Tech.Mean.Ct") %>%
  ...
  select(Genotype = sample_name, Y45F10D.4, `bm-osm-9` = `OSM-9`, `bm-tax-4` = `TAX-4`, `bm-ocr-1/2a` = `OCR-1/2a`) %>%
  mutate(Genotype = c("N2", "osm-9(ky10)", "", "", "", "", "", "",
                      "tax-4(p678)", "", "", "")) %>%
  mutate(`Y45F10D.4` = as.character(round(x = `Y45F10D.4`, digits = 2))) %>%
  mutate(`bm-osm-9` = c("ND", "ND", "23.88", "24.44", "23.47", "29.19", "26.91", "27.51", "", "", "", "")) %>%
  mutate(`bm-tax-4` = c("ND", "", "", "", "", "", "", "", "ND", "26.98", "34.38", "26.13")) %>%
  mutate(`bm-ocr-1/2a` = c("ND", "ND", "", "", "", "37.20", "34.30", "33.90", "", "", "", ""))
