...
RNAsamples.MF.M <- RNAsamples.MF %>%
  select(-AF) %>%
  filter(!is.na(AM))
RNAsamples.MF.F <- RNAsamples.MF %>%
  select(-AM) %>%
  filter(!is.na(AF)) %>%
  select(Transcript_ID, AF)
...
RNAsamples.HT.H <- RNAsamples.HT %>%
  select(-T) %>%
  filter(!is.na(H))
RNAsamples.HT.T <- RNAsamples.HT %>%
  select(-H) %>%
  filter(!is.na(T)) %>%
  select(Transcript_ID, T)
...
plot(hc,
  ...
  col.axis = "#F38630", lwd = 2.5, lty = 1, sub = "", axes = FALSE, cex = 0.5, ann = FALSE
)
# add axis
axis(
...
# saveRDS(dend, "~/Box/ZamanianLab/Manuscripts/2019-ChemoR/v3-PBIOrevisions/Figs/Fig2/dend.plot")
...
family.map <- ggplot(pd, aes(x = id2, y = row)) +
  geom_tile(aes(fill = Superfamily), width = 0.9) +
...
# heatmap_plot <- grid.arrange(dend + theme(plot.margin = unit(c(1, 2.3, -2, 0.8), "cm")), 
#                              heatmap +
#                                theme(axis.text.y = element_blank(),
#                                      axis.ticks.y = element_blank(),
#                                      theme(plot.margin = unit(c(1, 1, 1, 1), "cm")), ncol = 1, heights = c(0.3, 1)))

final.heatmap <- plot_grid(dend + theme(plot.margin = margin(b = -25)),
  family.map,
  heatmap + theme(
    axis.title.x = element_blank(),
